...
class SSHMockFactory:
    ...
    @staticmethod
    def create_transport(is_active: bool = True, is_authenticated: bool = True) -> Mock:
        """
        创建 Transport Mock
    ...
    def create_long_running_channel(
        data_chunks: Optional[List[bytes]] = None, exit_code: int = 0, delay_cycles: int = 3
    ) -> Mock:
        """
    ...
    @staticmethod
    def create_ssh_client(is_connected: bool = True, transport_active: bool = True) -> Mock:
        """
        创建 SSHClient Mock
        ...
        client = Mock()
        transport = SSHMockFactory.create_transport(
            is_active=transport_active, is_authenticated=is_connected
        )
    ...
    @staticmethod
    def create_connection_manager(is_connected: bool = True, transport_active: bool = True) -> Mock:
        """
        ...
        """
        manager = Mock()
        transport = SSHMockFactory.create_transport(is_active=transport_active)
    ...
    @staticmethod
    def create_connection_pool(pool_size: int = 5, available_connections: int = 3) -> Mock:
        """
    ...
    @staticmethod
    def create_receiver_config(recv_mode: str = "auto", timeout: float = 30.0) -> Mock:
        """
        创建 Receiver 配置 Mock
...
# 便捷函数
def create_mock_ssh_setup(stdout: bytes = b"", stderr: bytes = b"", exit_code: int = 0) -> tuple:
    """
    创建完整的 Mock SSH 设置
