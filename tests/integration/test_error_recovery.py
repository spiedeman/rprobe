...
class TestPoolErrorRecovery:
    ...
    def test_pool_exhaustion_error(self, test_environment):
        ...
        pool = ConnectionPool(config, max_size=2, min_size=1, acquire_timeout=1.0, health_check_interval=0)
...
class TestCommandErrorHandling:
    ...
    def test_permission_denied_handling(self, test_environment):
        ...
        with SSHClient(config) as client:
            # 尝试读取可能无权限的文件
            result = client.exec_command("cat /root/.bashrc 2>/dev/null || echo 'Permission denied'")
...
class TestNetworkResilience:
    ...
    def test_connection_with_large_payload(self, test_environment):
        ...
        with SSHClient(config) as client:
            # 生成大数据
            result = client.exec_command("dd if=/dev/zero bs=1024 count=100 | base64 | head -c 10000")
            assert result.exit_code == 0
