...
class TestBoundaryValueAnalysis:
    ...
    def test_boundary_idle_time_min(self, test_environment):
        ...
        pool = ConnectionPool(
            config, max_size=2, min_size=1, max_idle=0.05, health_check_interval=0  # 50ms
        )
...
class TestDecisionTable:
    ...
    def test_decision_table_4_has_unhealthy_connection(self, test_environment):
        ...
        pool = ConnectionPool(
            config, max_size=2, min_size=1, max_idle=0.05, health_check_interval=0  # 快速过期
        )
...
class TestErrorGuessing:
    ...
    def test_error_invalid_utf8_in_output(self, test_environment):
        ...
        with SSHClient(config) as client:
            # 生成包含各种字符的输出
            result = client.exec_command(
                "printf 'test\x80\x81\x82' 2>/dev/null || echo 'binary ok'"
            )
            # 应该能处理，不会崩溃
...
class TestScenarioTesting:
    ...
    def test_scenario_log_analysis(self, test_environment):
        ...
        with SSHClient(config) as client:
            ...
            # 查看最近的日志
            result = client.exec_command(
                "echo 'log entry 1'; echo 'log entry 2'; echo 'log entry 3'"
            )
    ...
    def test_scenario_backup_task(self, test_environment):
        ...
        with SSHClient(config) as client:
            # 1. 创建测试数据
            result = client.exec_command(
                "mkdir -p /tmp/backup_test && echo 'data' > /tmp/backup_test/file.txt"
            )
            assert result.exit_code == 0

            # 2. 打包备份
            result = client.exec_command(
                "tar -czf /tmp/backup.tar.gz -C /tmp backup_test && echo 'backup done'"
            )
    ...
    def test_scenario_multi_session_workspace(self, test_environment):
        ...
        try:
            ...
            build_output = build_session.execute_command(
                "echo 'Building project...' && echo 'Build complete'"
            )
            ...
            test_output = test_session.execute_command(
                "echo 'Running tests...' && echo 'Tests passed'"
            )
