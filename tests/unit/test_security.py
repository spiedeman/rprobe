...
class TestInputValidation:
    ...
    def test_command_injection_in_host(self, host):
        ...
        config = SSHConfig(host=host, username="user", password="pass")
    ...
    def test_username_injection_attempts(self, username):
        """测试用户名注入尝试"""
        config = SSHConfig(host="example.com", username=username, password="pass")
    ...
    def test_password_injection_attempts(self, password):
        """测试密码注入尝试"""
        config = SSHConfig(host="example.com", username="user", password=password)
...
class TestCredentialExposure:
    ...
    def test_password_not_in_str(self):
        """测试密码不出现在字符串表示中"""
        config = SSHConfig(host="example.com", username="user", password="secret123")
...
class TestPathValidation:
    ...
    def test_key_path_validation(self):
        ...
        with pytest.raises(ConfigurationError, match="密钥文件不存在"):
            SSHConfig(host="example.com", username="user", key_filename="/nonexistent/key/file")
    ...
    def test_path_traversal_prevention(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # 尝试使用相对路径
            config = SSHConfig(host="example.com", username="user", key_filename=allowed_key)
...
class TestResourceLimits:
    ...
    def test_max_output_size_limit(self):
        """测试最大输出大小限制"""
        config = SSHConfig(
            host="example.com", username="user", password="pass", max_output_size=1024  # 1KB
        )
    ...
    def test_port_range_limits(self):
        """测试端口范围限制"""
        # 有效边界值
        config1 = SSHConfig(host="example.com", username="user", password="pass", port=1)
        assert config1.port == 1

        config2 = SSHConfig(host="example.com", username="user", password="pass", port=65535)
...
class TestErrorHandling:
    ...
    def test_error_message_no_credential_leak(self):
        ...
        try:
            SSHConfig(host="", username="user", password=password)
