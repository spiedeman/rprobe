...
class TestSSHConfig:
    ...
    def test_valid_password_config(self):
        """测试有效的密码认证配置"""
        config = SSHConfig(host="example.com", username="user", password="pass123")
    ...
    def test_valid_key_config(self):
        ...
        try:
            config = SSHConfig(host="example.com", username="user", key_filename=key_path)
    ...
    def test_validation_both_auth_methods(self):
        ...
        with pytest.raises(ConfigurationError, match="密码和密钥不能同时指定"):
            SSHConfig(
                host="example.com", username="user", password="pass", key_filename="/path/to/key"
            )
    ...
    def test_validation_missing_key_file(self):
        ...
        with pytest.raises(ConfigurationError, match="密钥文件不存在"):
            SSHConfig(host="example.com", username="user", key_filename="/nonexistent/key")
...
class TestConfigManager:
    ...
    def test_build(self):
        """测试构建配置对象"""
        manager = ConfigManager()
        manager.from_dict({"host": "test.com", "username": "admin", "password": "secret"})
    ...
    def test_load_from_file_classmethod(self):
        """测试类方法从文件加载"""
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            json.dump({"host": "file.com", "username": "fileuser", "password": "filepass"}, f)
            json_path = f.name
    ...
    def test_create_default(self):
        ...
        config = ConfigManager.create_default(
            host="default.com", username="defaultuser", password="defaultpass", port=2222
        )
...
class TestLoadConfig:
    ...
    def test_load_config_from_kwargs(self):
        """测试从kwargs加载配置"""
        config = load_config(
            use_env=False, host="kwarg.com", username="kwarguser", password="kwargpass"
        )
    ...
    def test_load_config_priority(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            json.dump(
                {"host": "file.com", "username": "fileuser", "password": "filepass", "port": 1111},
                f,
            )
            json_path = f.name

        # 环境变量配置
        env_vars = {"REMOTE_SSH_PORT": "2222"}  # 覆盖文件的1111

        try:
            with patch.dict(os.environ, env_vars, clear=False):
                # kwargs优先级最高，覆盖环境变量
                config = load_config(file_path=json_path, use_env=True, port=3333)  # 最高优先级
