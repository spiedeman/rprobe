...
class TestMultipleShellSessionsAPI:
    ...
    def test_session_management_methods(self, mock_ssh_config):
        ...
        # 添加会话
        client._shell_sessions["session1"] = mock_session
        client._default_session_id = "session1"
        ...
        # 获取会话
        session = client.get_shell_session("session1")
    ...
    def test_set_default_session(self, mock_ssh_config):
        ...
        client._shell_sessions["session1"] = mock_session1
        client._shell_sessions["session2"] = mock_session2
        client._default_session_id = "session1"
        
        assert client._default_session_id == "session1"
        
        # 设置新的默认会话
        client.set_default_shell_session("session2")
        assert client._default_session_id == "session2"
    ...
    def test_close_all_sessions(self, mock_ssh_config):
        ...
        for i in range(3):
            mock_session = Mock()
            mock_session.is_active = True
            client._shell_sessions[f"session{i}"] = mock_session
        
        client._default_session_id = "session0"
    ...
    def test_backward_compatibility(self, mock_ssh_config):
        ...
        client._shell_sessions[session_id] = mock_session
        client._default_session_id = session_id
