...
def mock_config():
    """创建测试配置"""
    return SSHConfig(host="test.example.com", username="testuser", password="testpass")
...
class TestSmartChannelReceiverRecvAll:
    ...
    def test_recv_all_with_select_mode(self, mock_config):
        ...
        with patch.object(
            receiver._receiver, "recv_all_optimized", side_effect=TypeError("mock error")
        ):
            with patch(
                "src.receivers.channel_receiver_optimized.AdaptivePollingReceiver"
            ) as mock_adaptive_cls:
                mock_adaptive = Mock()
    ...
    def test_recv_all_uses_default_timeout(self, mock_config):
        ...
        with patch.object(
            receiver._receiver, "recv_all", return_value=("stdout", "stderr", 0)
        ) as mock_recv:
