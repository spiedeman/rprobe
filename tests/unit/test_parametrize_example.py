...
class TestCartesianProduct:
    ...
    @pytest.mark.parametrize(
        "recv_mode", ["auto", "select", "adaptive", "original"]
    )
    @pytest.mark.parametrize(
        "encoding", ["utf-8", "gbk", "latin-1"]
    )
...
class TestDynamicParams:
    ...
    @pytest.mark.parametrize(
        "port",
        generate_port_range(),
        ids=lambda p: f"port_{p}"  # 自定义测试ID
    )
...
class TestParametrizeWithFixture:
    ...
    def test_config_with_extra_params(
        self, base_config_dict, extra_config, expected_value
    ):
        """测试基础配置加额外参数"""
...
class TestSkipSpecificParams:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                "windows", "select",   # Windows不支持select
                marks=pytest.mark.skip(
                    reason="Windows不支持select模式"
                )
            ),
...
@pytest.mark.skipif(
    not hasattr(SSHConfig, 'recv_mode'),
    reason="SSHConfig不支持recv_mode属性"
)
...
if __name__ == "__main__":
    ...
    pytest.main([
        ...
        "-v",
        "--tb=short",
        "-k", "TestParametrized or TestMultiParametrize"
    ])
