...
class AdaptiveWaitStrategy:
    ...
    def wait(self) -> float:
        ...
        # 计算下一次等待时间
        self._current_wait = min(self._current_wait * self.growth_factor, self.max_wait)
...
class BlockingWaitStrategy:
    ...
    def wait_for_data(
        self, recv_func: Callable[[int], bytes], timeout: Optional[float] = None
    ) -> Optional[bytes]:
        """
        阻塞等待数据到达
...
# 便捷函数：计算平均等待间隔
def calculate_average_wait(strategy: AdaptiveWaitStrategy, iterations: int = 100) -> float:
    """
    计算给定迭代次数的平均等待时间
