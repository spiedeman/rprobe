...
class ShellSession:
    ...
    def __init__(
        self, channel: Channel, config: SSHConfig, prompt_detector: Optional[PromptDetector] = None
    ):
        """
        初始化 Shell 会话
    ...
    def initialize(self, timeout: Optional[float] = None) -> str:
        ...
        while time.time() - start_time < timeout:
            if self._channel.recv_ready():
                data = self._channel.recv(4096).decode(self._config.encoding, errors="replace")
                initial_output += data
    ...
    def _wait_for_output(self, timeout: Optional[float] = None) -> str:
        ...
        while True:
            ...
            if output_bytes and (time.time() - last_data_time) > 0.1:
                clean_output = ANSICleaner.clean(
                    output_bytes.decode(self._config.encoding, errors="replace")
                )
                lines = clean_output.strip().split("\n")
                if lines:
                    last_line = lines[-1].strip()
    ...
    def enter_interactive(
        self, program: str, prompt: Optional[str] = None, learn_prompts: Optional[list] = None
    ) -> str:
        """
        进入交互式程序前保存当前上下文
