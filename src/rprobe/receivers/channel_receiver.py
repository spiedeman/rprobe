...
class ChannelDataReceiver:
    ...
    def recv_once(
        self, channel: Channel, is_stderr: bool = False, current_size: int = 0
    ) -> Tuple[bytes, bool]:
        """
        从通道读取单次数据
    ...
    def recv_all(
        ...
        while True:
            ...
            # 读取 stdout 数据
            try:
                data, truncated = self.recv_once(
                    channel, is_stderr=False, current_size=len(stdout_data)
                )
                if data:
                    stdout_data += data
            ...
            try:
                data, truncated = self.recv_once(
                    channel, is_stderr=True, current_size=len(stderr_data)
                )
                if data:
                    stderr_data += data
                    last_activity_time = time.time()
        ...
        logger.debug(
            f"数据接收完成: stdout={len(stdout_text)}字符, stderr={len(stderr_text)}字符, exit_code={exit_code}"
        )
