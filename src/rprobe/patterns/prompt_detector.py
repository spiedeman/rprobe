...
class PromptDetector:
    ...
    def match_line(self, line: str) -> PromptMatchResult:
        ...
        for i, learned_pattern in enumerate(reversed(self._learned_patterns)):
            if learned_pattern.search(line):
                ...
                learned_prompt = (
                    self._learned_prompts[actual_index]
                    if actual_index < len(self._learned_prompts)
                    else ""
                )
                result = PromptMatchResult(is_match=True, matched_text=line, is_learned=True)
    ...
    def _learn_prompt(self, prompt: str) -> None:
        ...
        if learned:
            ...
            self._learned_prompts.append(prompt)
            logger.debug(
                f"学习了新的提示符模式: {prompt} (当前共 {len(self._learned_patterns)} 个)"
            )
    ...
    def _strip_prompt_line(self, lines: List[str]) -> List[str]:
        ...
        if self._last_prompt:
            if (
                last_line == self._last_prompt
                or last_line.startswith(self._last_prompt)
                or self._last_prompt in last_line
            ):
                return lines[:-1]
