...
class PoolStatsCollector:
    ...
    def get_stats(
        ...
        Args:
            ...
            max_size: 最大连接数
            human_readable: 是否使用人类可读格式（默认True）
        ...
        with self._lock:
            ...
            # 计算平均等待时间
            avg_wait_time = metrics.total_wait_time / metrics.waits if metrics.waits > 0 else 0.0

            # 计算获取时间统计
            avg_acquire_time = (
                sum(self._acquire_times) / len(self._acquire_times) if self._acquire_times else 0.0
            )
            ...
            init_success_rate = (
                metrics.init_succeeded / init_attempts * 100 if init_attempts > 0 else 0.0
            )

            health_check_rate = (
                metrics.health_check_passed / metrics.health_checks * 100
                if metrics.health_checks > 0
                else 0.0
            )

            total_acquired = metrics.created + metrics.reused
            reuse_rate = metrics.reused / total_acquired * 100 if total_acquired > 0 else 0.0

            avg_lifetime = (
                sum(self._connection_lifetimes) / len(self._connection_lifetimes)
                if self._connection_lifetimes
                else 0.0
            )
            ...
            stats.update(
                {
                    ...
                    "init_succeeded": metrics.init_succeeded,
                    "init_failed": metrics.init_failed,
                    "health_checks": metrics.health_checks,
                    "health_check_passed": metrics.health_check_passed,
                    "health_check_failed": metrics.health_check_failed,
                    "closed_during_shutdown": metrics.closed_during_shutdown,
            ...
            if human_readable:
                ...
                stats.update(
                    {
                        "avg_lifetime": self._format_duration(avg_lifetime),
                        "avg_wait_time": self._format_duration(avg_wait_time),
                        "avg_wait_time_recent": self._format_duration(avg_wait_time_recent),
                        "avg_acquire_time": self._format_duration(avg_acquire_time),
                        "max_acquire_time": self._format_duration(max_acquire_time),
                        "total_wait_time": self._format_duration(metrics.total_wait_time),
                        "uptime": self._format_uptime(uptime_seconds),
                        "created_at": self._format_timestamp(metrics.created_at),
                        "last_activity": self._format_relative_time(metrics.last_activity),
                    }
                )
            else:
                # 原始数值格式
                stats.update(
                    {
                        ...
                        "total_wait_time": round(metrics.total_wait_time, 3),
